...
# Generate these by running: python -m src.generate_api_key
...
2. Run: `python -m src.generate_api_key`
...
- Set `USE_RICH_OUTPUT=true` for better console output (requires `pip install rich`)
...
3. **Validate** with `python -m src.diagnose_config` if needed
