...
2. **Wrong POLYMARKET_FUNDER**
   ...
   - Run `python -m src.diagnose_config` to verify

3. **API Credentials Mismatch**
   - Regenerate API keys: `python -m src.generate_api_key`
...
4. Run `python -m src.diagnose_config` for details
...
1. **For Magic.link Users**:
   ...
   - Run `python -m src.test_balance` to verify
...
4. **Verify balance** with `python -m src.test_balance`
...
3. **Check configuration**: Run `python -m src.diagnose_config`
4. **Check logs**: Look for error messages
5. **Verify imports**: Try `python -c "from src.config import load_settings"`
...
2. **Run diagnostics**: `python -m src.diagnose_config`
...
- [ ] Run `python -m src.diagnose_config` before first use
- [ ] Test in simulation mode first (`DRY_RUN=true`)
- [ ] Verify balance with `python -m src.test_balance`
